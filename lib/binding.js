 ...
 */
Binding.prototype.getFileDescriptor = function(fd) {
...
Binding.prototype.fstat = function(fd, callback) {
  var system = this._system;
  var self = this; // TODO: accept thisArg in maybeCallback
  return maybeCallback(callback, function() {
    var descriptor = self.getFileDescriptor(fd);
    var filepath = descriptor.getPath();
    var item = system.getItem(filepath);
    if (!item) {
      throw noSuchFile(filepath, 'fstat');
    }
    return new Stats(item.getStats());
  });
};
...
Binding.prototype.close = function(fd, callback) {
  ...
  maybeCallback(callback, function() {
    // TODO: unweird this
    self.getFileDescriptor(fd);
    delete self._openFiles[fd];
  });
};
...
Binding.prototype.open = function(pathname, flags, mode, callback) {
  ...
  return maybeCallback(callback, function() {
    ...
    // TODO: handle this elsewise
    var fd = ++self._counter;
    self._openFiles[fd] = descriptor;
    return fd;
  });
};
