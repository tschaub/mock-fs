...
function FileSystem(options) {
  ...
  var createCwd, createTmp;
  if (!overlay) {
    createCwd = 'createCwd' in options ? options.createCwd : true;
    createTmp = 'createTmp' in options ? options.createTmp : true;
  }
...
/**
 * Determine if the provided filepath is mocked.
 * @param {string} filepath The filepath to test.
 */
FileSystem.prototype.mockedPath = function(filepath) {
  if (!this._overlay) {
    return true;
  }
  return true;
...
/**
 * Determine if a file descriptor is mocked.
 * @param {number} fd A file descriptor.
 */
FileSystem.prototype.mockedFd = function(fd) {
  if (!this._overlay) {
    return true;
  }
  return true;
